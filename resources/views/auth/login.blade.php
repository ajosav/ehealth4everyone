...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Ehealth4everyone') }}</title>
        ...
        <style>
            ...
            * {
            ...
            .panel input[type='text'][placeholder], input[type='text']['placeholder']{
                color: #656d79;
                font-size: 15px;
                font-weight: 500;
        ...
        <div class="panel">
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="form">
                    ...
                    @endif
                    <input placeholder='Email/Phone' type="text"   id="email" type="text" class="{{$errors->has('phone') || $errors->has('email') ? 'is-invalid' : ''}}" name="email_or_phone" value="{{old('phone') ?: old('email') }}" required autofocus>
            ...
            <div class="fack">
                @if (Route::has('password.request'))
                    <a class="btn btn-link" style="margin-right: 8px" href="{{ route('password.request') }}">
                        <i class="fa fa-question-circle"></i>{{ __('Forgot Your Password?') }}
                    </a>
                @endif
                <a href="{{url('/register')}}" style="margin-left: 4px; text-decoration: none">New User? <span style="color: #007bff">Sign Up</span></a>
            </div>
        </div>
